...
import {
    UpdateBidListContext, UpdateCarbonEmissionContext,
    UpdateHistoryEventsContext, UpdatePersonalInfoContext, UpdateProductListContext,
...
export const Entrepreneur = ({ent_id}) => {
    ...
    const fight_for_bid = (project_id, database) => {
        ...
        else if(entrepreneur.projects_in_progress===2){
            setSeverity("warning")
            setAlertMessage("该企业已达到项目数量上限,请完成项目后继续竞标(2个)")
            setSnackBar(true)
            return
        }
        ...
        updateHistoryEvents("企业"+ent_id+"成功竞标了"+project_id+"项目")
        setSnackBar(true)
    }
    ...
    const sellProduct=(product_id,database)=>{
        ...
        if(currentPolicyId==="G01"&&(!product.isGreen)){
            setSeverity("warning")
        ...
        if(entrepreneur.money<product.producing_cost){
            setSeverity("warning")
            setAlertMessage("资金不足，无法生产")
            setSnackBar(true)
            return
        }
        if(productList.some((prod)=>{
            return prod.productId===product_id;
        })){
            setSeverity("error")
            setAlertMessage("该商品已经在售卖")
            setSnackBar(true)
            return
        }
        productList.push({
    ...
    return (
        <>
        <InfoCard dataName={"entrepreneur_" + ent_id} actionA={"竞标"} actionB={"卖商品"} databaseA={"projects"} databaseB={"products"} funcA={fight_for_bid} funcB={sellProduct} disableSnackBar={true}></InfoCard>
